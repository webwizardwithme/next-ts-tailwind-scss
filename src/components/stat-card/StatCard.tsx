...
function StatCardComponent(
  ...
  const cardProps = isInteractive
    ? mergeProps(buttonProps, hoverProps, focusProps, tooltipForwardProps)
    : tooltipForwardProps;
        ...
        <Badge variant='primary' className={el`badge`} size='small'>
          {/* {p.variant !== 'default' && (
            <Icon
              content={
                p.variant === 'positive'
                  ? POSITIVE_VARIANT_ICON
                  : NEGATIVE_VARIANT_ICON
              }
        ...
        {isOnlyTitleBadge ? (
          <section className={el`content`}>
            <div className={el`title-badge`}>
              <Text type='h5' className={el`title`}>
                {p.title}
              </Text>
              {badge}
            </div>
          ...
          <>
            ...
            {trends.length > 0 && (
              <section
                className={isBadgeExist ? el`trend-end` : el`trend-center`}
              >
